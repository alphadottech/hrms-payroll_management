...
public class DetailedSalarySlip {
	...
	public ByteArrayOutputStream generateDetailedSalarySlipPDF(String empId, String name, int totalWorkingDays,
		...
		companyName.add(new Phrase("Alpha Dot Technologies Pvt ltd.",
		...
		// companyName.add(new Paragraph("Salary
		// SLip",FontFactory.getFont(FontFactory.HELVETICA_BOLD,16,BaseColor.RED)));
		...
		canvas.lineTo(document.getPageSize().getWidth() - 50, document.getPageSize().getHeight() - 132); // Adjust
																											// position
																											// as needed
...
// Draw a smaller rectangle around the content
		...
		PdfPTable earningsTable = createParallelTable("Earnings", "Basic Salary", String.valueOf(basic), "Hra",
				String.valueOf(hra), "Adhoc", String.valueOf(adhoc1));
		...
		// PdfPTable deductionsTable = createParallelTable("Deductions", "Esic
		// Deduction", String.valueOf(esicAmount), "PF Deduction",
		// String.valueOf(pfAmount),"Adjustment",String.valueOf(adjustment), "Unpaid
		// Leave", String.valueOf(Math.round(unpaidLeave)));
		PdfPTable deductionsTable = medicalInsurance == 0
				? createParallelTable("Deductions", "Esic Deduction", String.valueOf(esicAmount), "PF Deduction",
						String.valueOf(pfAmount), "Adjustment", String.valueOf(adjustment), "Absent Deduction",
						String.valueOf(Math.round(unpaidLeave - halfDayAmount)), "TDS", String.valueOf(tds))
				: createParallelTable("Deductions", "Esic Deduction", String.valueOf(esicAmount), "PF Deduction",
						String.valueOf(pfAmount), "Adjustment", String.valueOf(adjustment), "Absent Amount",
						String.valueOf(Math.round(unpaidLeave)), "Medical Insurance", String.valueOf(medicalInsurance),
